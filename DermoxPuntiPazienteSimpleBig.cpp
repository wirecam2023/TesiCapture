#include "stdafx.h"
#include "Endox.h"
#include "DermoxPuntiPazienteSimpleBig.h"
...
IMPLEMENT_DYNAMIC(CDermoxPuntiPazienteSimpleBig, CEndoxResizableDlg)

CDermoxPuntiPazienteSimpleBigDlg::CDermoxPuntiPazienteSimpleBigDlg(CWnd* pParent, CEsamiView* pEsamiView, int iCurSelPointIndex)
: CEndoxResizableDlg(CDermoxPuntiPazienteSimpleBigDlg::IDD, pParent)
{
...
CDermoxPuntiPazienteSimpleBigDlg::~CDermoxPuntiPazienteSimpleBigDlg()
{
}

BEGIN_MESSAGE_MAP(CDermoxPuntiPazienteSimpleBigDlg, CEndoxResizableDlg)
	ON_WM_LBUTTONDOWN()
	ON_WM_MOUSEMOVE()
	ON_WM_PAINT()
	ON_WM_SIZE()
...
void CDermoxPuntiPazienteSimpleBigDlg::DoDataExchange(CDataExchange* pDX)
{
...
void CDermoxPuntiPazienteSimpleBigDlg::OnCancel()
{
	if (m_nOperazione == 1)
	{
		//
...
BOOL CDermoxPuntiPazienteSimpleBigDlg::OnInitDialog()
{
	...
	if (bOk)
	{
		...
		AddAnchor(IDC_STATIC_58, CSize(0, 0), CSize(100, 0));
		...
		if (setPunti.OpenRecordset("CDermoxPuntiPazienteSimpleBigDlg::OnInitDialog"))
		{
			...
			setPunti.CloseRecordset("CDermoxPuntiPazienteSimpleBigDlg::OnInitDialog");
		}

		//
		CPuntiSediSet setSedi;
		if (setSedi.OpenRecordset("CDermoxPuntiPazienteSimpleBigDlg::OnInitDialog"))
		{
			while (!setSedi.IsEOF())
			{
				tagSede tagTemp;
			...
			setSedi.CloseRecordset("CDermoxPuntiPazienteSimpleBigDlg::OnInitDialog");
		}
		...
		//
		theApp.LocalizeDialog(this, CDermoxPuntiPazienteSimpleBigDlg::IDD, "DermoxPuntiPazienteSimpleDlg");
	}
	else
	{
...
void CDermoxPuntiPazienteSimpleBigDlg::OnOK()
{
}

void CDermoxPuntiPazienteSimpleBigDlg::OnBnClickedBtnExit()
{
	if (m_bModified)
	{
...
/*
void CDermoxPuntiPazienteSimpleBigDlg::OnBnClickedRadioSel()
{
UpdateData(TRUE);
...
void CDermoxPuntiPazienteSimpleBigDlg::OnBnClickedRadioMov()
{
	UpdateData(TRUE);
...
void CDermoxPuntiPazienteSimpleBigDlg::OnBnClickedRadioDel()
{
	UpdateData(TRUE);
...
void CDermoxPuntiPazienteSimpleBigDlg::OnBnClickedBtnConfronto()
{
	CAlbumConfrontoDermoxDlg dlg(this, m_pEsamiView, CPoint(m_iCurSelPointX, m_iCurSelPointY));
	dlg.DoModal();
}

void CDermoxPuntiPazienteSimpleBigDlg::OnBnClickedCheck()
{
...
void CDermoxPuntiPazienteSimpleBigDlg::OnBnClickedRadio()
{
...
void CDermoxPuntiPazienteSimpleBigDlg::OnCbnSelchangeCombo()
{
...
void CDermoxPuntiPazienteSimpleBigDlg::OnEnChangeEdit()
{
...
void CDermoxPuntiPazienteSimpleBigDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
	point.x++;
	point.y++;
...
void CDermoxPuntiPazienteSimpleBigDlg::OnMouseMove(UINT nFlags, CPoint point)
{
...
void CDermoxPuntiPazienteSimpleBigDlg::OnLvnItemchangedListPoints(NMHDR *pNMHDR, LRESULT *pResult)
{
	if (!m_bFillingList)
	{
		BeginWaitCursor();
...
void CDermoxPuntiPazienteSimpleBigDlg::OnLvnItemchangingListPoints(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CDermoxPuntiPazienteSimpleBigDlg::OnNMDblClkListImages(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CDermoxPuntiPazienteSimpleBigDlg::OnPaint()
{
	CPaintDC dc(this);
...
void CDermoxPuntiPazienteSimpleBigDlg::OnSize(UINT nType, int cx, int cy)
{
...
CPoint CDermoxPuntiPazienteSimpleBigDlg::ImageToScreen(CPoint ptPoint)
{
	CPoint ptReturn;
...
CPoint CDermoxPuntiPazienteSimpleBigDlg::ImageToScreenZoom(CPoint ptPoint, CRect rectImageZoom)
{
	CPoint ptReturn;
...
CPoint CDermoxPuntiPazienteSimpleBigDlg::ScreenToImage(CPoint ptPoint)
{
	CPoint ptReturn;
...
void CDermoxPuntiPazienteSimpleBigDlg::CaricaDatiPunto()
{
	// ID del punto //
	long lIDPunto = 0;
	if (m_iCurSelPointIndex >= 0)
	...
	if (lIDPunto > 0)
	{
		...
		if (setPP.OpenRecordset("CDermoxPuntiPazienteSimpleBigDlg::CaricaDatiPunto"))
		{
			if (!setPP.IsEOF())
			{
				m_bLoadingData = TRUE;
			...
			setPP.CloseRecordset("CDermoxPuntiPazienteSimpleBigDlg::CaricaDatiPunto");
		}
	}
...
void CDermoxPuntiPazienteSimpleBigDlg::CaricaImmaginiPunto()
{
	// ID del punto //
	long lIDPunto = 0;
	if (m_iCurSelPointIndex >= 0)
...
void CDermoxPuntiPazienteSimpleBigDlg::EliminaPunto()
{
	// ID del punto //
	long lIDPunto = 0;
	if (m_iCurSelPointIndex >= 0)
	...
	if (lIDPunto > 0)
	{
		...
		CPuntiPazienteSet setPP;
		setPP.SetOpenFilter(strFilter);
		if (setPP.OpenRecordset("CDermoxPuntiPazienteSimpleBigDlg::EliminaPunto"))
		{
			if (!setPP.IsEOF())
			{
				if (setPP.DeleteRecordset("CDermoxPuntiPazienteSimpleBigDlg::EliminaPunto"))
				{
			...
			setPP.CloseRecordset("CDermoxPuntiPazienteSimpleBigDlg::EliminaPunto");
		}
	}
}

void CDermoxPuntiPazienteSimpleBigDlg::RiempiCombo09()
{
...
void CDermoxPuntiPazienteSimpleBigDlg::SalvaDatiPunto()
{
	// ID del punto //
	long lIDPunto = 0;
	if (m_iCurSelPointIndex >= 0)
	...
	if (lIDPunto > 0)
	{
		...
		if (setPP.OpenRecordset("CDermoxPuntiPazienteSimpleBigDlg::CaricaDatiPunto"))
		{
			if (!setPP.IsEOF())
			{
				if (setPP.EditRecordset("CDermoxPuntiPazienteSimpleBigDlg::CaricaDatiPunto"))
				{
					...
					if (setPP.UpdateRecordset("CDermoxPuntiPazienteSimpleBigDlg::CaricaDatiPunto"))
					{
						CString strTemp;
...
void CDermoxPuntiPazienteSimpleBigDlg::SelezionaPunto(CPoint ptPoint)
{
	BeginWaitCursor();
...
void CDermoxPuntiPazienteSimpleBigDlg::SpostaPunto(CPoint ptPoint)
{
	BeginWaitCursor();
	...
	if (nIndex < 0)
	{
		...
		if (theApp.AfxMessageBoxEndo(strMessage, MB_YESNO | MB_ICONQUESTION) == IDYES)
		{
			...
			if (lIDPunto > 0)
			{
				...
				CPuntiPazienteSet setPP;
				setPP.SetOpenFilter(strFilter);
				if (setPP.OpenRecordset("CDermoxPuntiPazienteSimpleBigDlg::SpostaPunto"))
				{
					if (!setPP.IsEOF())
					{
						if (setPP.EditRecordset("CDermoxPuntiPazienteSimpleBigDlg::SpostaPunto"))
						{
							setPP.m_lX = ptPoint.x;
							setPP.m_lY = ptPoint.y;
							setPP.m_sSede = m_sSedeNew;

							if (setPP.UpdateRecordset("CDermoxPuntiPazienteSimpleBigDlg::SpostaPunto"))
							{
					...
					setPP.CloseRecordset("CDermoxPuntiPazienteSimpleBigDlg::SpostaPunto");
				}
			}
		}
		else
...
int CDermoxPuntiPazienteSimpleBigDlg::TrovaPunto(CPoint& ptPoint)
{
	int nIndex = 0;
...
void CDermoxPuntiPazienteSimpleBigDlg::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
BOOL CDermoxPuntiPazienteSimpleBigDlg::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	// TODO: Add your message handler code here and/or call default
	if (zDelta == -120)
	{
		CDermoxPuntiPazienteSimpleBigDlg::OnVScroll(SB_PAGEDOWN, 0, NULL);
		return TRUE;
	}
	else if (zDelta == 120)
	{
		CDermoxPuntiPazienteSimpleBigDlg::OnVScroll(SB_PAGEUP, 0, NULL);
		return TRUE;
	}
	else
		return TRUE;
