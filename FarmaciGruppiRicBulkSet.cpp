...
IMPLEMENT_DYNAMIC(CFarmaciGruppiRicBulkSet, CBaseSet)

CFarmaciGruppiRicBulkSet::CFarmaciGruppiRicBulkSet()
...
CString CFarmaciGruppiRicBulkSet::GetDefaultSQL()
{
	return "FarmaciGruppiRic";
}

void CFarmaciGruppiRicBulkSet::DoBulkFieldExchange(CFieldExchange* pFX)
{
...
void CFarmaciGruppiRicBulkSet::CaricaInMemoria()
{
	theApp.m_listGruppiFarmaci.RemoveAll();
		...
		while(!IsEOF())
		{
			for(UINT i = 0; i < GetRowsFetched(); i++)
			{
				if (m_pIDLength[i] > 0)
				{
					...
					theApp.m_listGruppiFarmaci.AddTail(tagTemp);
				}
			}
...
CString CFarmaciGruppiRicBulkSet::GetTextField(LPTSTR szField, long* pFieldSize, int iIndex)
{
	CString strReturn;
