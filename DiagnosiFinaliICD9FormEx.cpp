...
CDiagnosiFinaliICD9FormEx::CDiagnosiFinaliICD9FormEx(CWnd* pParent, CEsamiView* pEsamiView)
	: CSubForm(CDiagnosiFinaliICD9FormEx::IDD, pParent, pEsamiView)
{
...
CDiagnosiFinaliICD9FormEx::~CDiagnosiFinaliICD9FormEx()
{
}

BEGIN_MESSAGE_MAP(CDiagnosiFinaliICD9FormEx, CSubForm)
	ON_WM_CTLCOLOR()
...
void CDiagnosiFinaliICD9FormEx::DoDataExchange(CDataExchange* pDX)
{
...
void CDiagnosiFinaliICD9FormEx::CreateFormEx(CWnd* pParent, CWnd* pParentFrame, CEsamiView* pEsamiView, long lColore, long lIDFaseBlocco)
{
	if (pEsamiView->m_pDynForm[sub_diagnosifinaliicd9ex] == NULL)
	{
		pEsamiView->m_pDynForm[sub_diagnosifinaliicd9ex] = new CDiagnosiFinaliICD9FormEx(NULL, pEsamiView);
		if (pEsamiView->m_pDynForm[sub_diagnosifinaliicd9ex] != NULL)
		{
			pEsamiView->m_pDynForm[sub_diagnosifinaliicd9ex]->m_pParentFrame = pParentFrame;
			pEsamiView->m_pDynForm[sub_diagnosifinaliicd9ex]->m_lColore = lColore;
			pEsamiView->m_pDynForm[sub_diagnosifinaliicd9ex]->m_lIDFaseBlocco = lIDFaseBlocco;

			CRect rectFrame;
			pParentFrame->GetWindowRect(&rectFrame);
			pEsamiView->m_pDynForm[sub_diagnosifinaliicd9ex]->CreateChildForm(CDiagnosiFinaliICD9FormEx::IDD, pParent, rectFrame);
		}
	}
}

HBRUSH CDiagnosiFinaliICD9FormEx::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
BOOL CDiagnosiFinaliICD9FormEx::OnInitDialog()
{
	...
	m_ctrlStatic.SetWindowText(theApp.GetMessageString(IDS_DIAGNOSIFINALE_ICD9));
...
void CDiagnosiFinaliICD9FormEx::ModificaForm()
{
	for (int i = 0; i < m_nIndexPage; i++)
		m_ctrlDiagnosi[i].SetParam(this, &m_ctrLine[m_nIndexLine + i].m_Set.m_lIDEsame, &m_ctrLine[m_nIndexLine + i].m_Set.m_lIDDiagnosi);
}

LRESULT CDiagnosiFinaliICD9FormEx::OnFormAnnullaEsame(WPARAM wParam, LPARAM lParam)
{
	return OnFormReset(wParam, lParam);
}

LRESULT CDiagnosiFinaliICD9FormEx::OnFormClone(WPARAM wParam, LPARAM lParam)
{
	...
	if (lEsame > 0)
	{
		...
		CDiagnosiFinaliEsamiSet setDiagnosi;
		setDiagnosi.SetOpenFilter(strFilter);
		setDiagnosi.SetSortRecord("ID");
		if (setDiagnosi.OpenRecordset("CDiagnosiFinaliICD9FormEx::OnFormClone"))
		{
			int nIndex = 0;
			...
			setDiagnosi.CloseRecordset("CDiagnosiFinaliICD9FormEx::OnFormClone");
		}
	}
...
LRESULT CDiagnosiFinaliICD9FormEx::OnFormLoad(WPARAM wParam, LPARAM lParam)
{
	OnFormReset(0, 0);
	GoToFirst();
	...
	if (m_pEsamiView->m_pEsamiSet->IsOpen() && !m_pEsamiView->m_pEsamiSet->IsFieldNull(&m_pEsamiView->m_pEsamiSet->m_lContatore))
	{
		...
		CDiagnosiFinaliEsamiSet setTemp;
		setTemp.SetOpenFilter(sFilter);
		if (setTemp.OpenRecordset("CDiagnosiFinaliICD9FormEx::OnFormLoad"))
		{
			int nCounter = 0;
			...
			setTemp.CloseRecordset("CDiagnosiFinaliICD9FormEx::OnFormLoad");
		}
	}
...
LRESULT CDiagnosiFinaliICD9FormEx::OnFormReadOnly(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CDiagnosiFinaliICD9FormEx::OnFormReset(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CDiagnosiFinaliICD9FormEx::OnFormSave(WPARAM wParam, LPARAM lParam)
{
	CDiagnosiFinaliEsamiSet setTemp;
	setTemp.SetOpenFilter("ID=0");
	if (setTemp.OpenRecordset("CDiagnosiFinaliICD9FormEx::OnFormSave"))
	{
		CSubForm::SaveForm(&m_pEsamiView->m_pEsamiSet->m_lContatore, &setTemp, &setTemp.m_lID, &setTemp.m_lIDEsame, "ID");
		setTemp.CloseRecordset("CDiagnosiFinaliICD9FormEx::OnFormSave");
	}

	return 1;
}

LRESULT CDiagnosiFinaliICD9FormEx::OnFormSearchNew(WPARAM wParam, LPARAM lParam)
{
	CString *pStrFrom = (CString*)lParam;
	CString *pStrWhere = (CString*)wParam;

	CDiagnosiFinaliEsamiSet* pSet = &m_ctrLine[0].m_Set;

	if (pSet)
	{
		ComponiRicercaFromEx(pStrWhere, pStrFrom, m_ctrlDiagnosi[0].m_strRicerca, "DiagnosiFinaliEsami", 0, "IdDiagnosi", "IdEsame", TRUE, TRUE);
	}

	return 1;
}

long CDiagnosiFinaliICD9FormEx::GetCounterSetOfLine(int nCounter)
{
	return m_ctrLine[nCounter].m_lID;
}

BOOL CDiagnosiFinaliICD9FormEx::IsLineDirty(int nCounter)
{
	return (m_ctrLine[nCounter].m_nStato & RECORD_DIRTY);
}

BOOL CDiagnosiFinaliICD9FormEx::IsValidSet(int nCounter)
{
	return (CDiagnosiICD9Set().GetStringDescrizione(m_ctrLine[nCounter].m_Set.m_lIDDiagnosi) != "");
}

void CDiagnosiFinaliICD9FormEx::SetLineDirty(int nCounter, BOOL bSetDirty)
{
	static UINT nID_Diagnosi[3] = { IDC_DIAGNOSI_1, IDC_DIAGNOSI_2, IDC_DIAGNOSI_3 };
...
void CDiagnosiFinaliICD9FormEx::UpdateDataLine(CBaseSet* pBaseSet, int nCounter, BOOL bToSet)
{
	CDiagnosiFinaliEsamiSet* pSet = (CDiagnosiFinaliEsamiSet*)pBaseSet;
	if (bToSet)
	{
...
void CDiagnosiFinaliICD9FormEx::OnChangeDiagnosi1()
{
	SetLineDirty(m_nIndexLine + 0, TRUE);
}

void CDiagnosiFinaliICD9FormEx::OnChangeDiagnosi2()
{
	SetLineDirty(m_nIndexLine + 1, TRUE);
}

void CDiagnosiFinaliICD9FormEx::OnChangeDiagnosi3()
{
	SetLineDirty(m_nIndexLine + 2, TRUE);
}

long CDiagnosiFinaliICD9FormEx::GetContenuto()
{
	return sub_diagnosifinaliicd9ex;
}

BOOL CDiagnosiFinaliICD9FormEx::CampoObbligatorioValido()
{
	for (int i = 0; i < MAX_OBJ_X_FORM; i++)
