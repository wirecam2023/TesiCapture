...
CRegioneForm::CRegioneForm(CWnd* pParent, CEsamiView* pEsamiView)
	: CSubForm(CRegioneForm::IDD, pParent, pEsamiView)
{
	m_nIndexLine = 0;
	m_nIndexPage = 3;
...
CRegioneForm::~CRegioneForm()
{
}

BEGIN_MESSAGE_MAP(CRegioneForm, CSubForm)
	ON_WM_CTLCOLOR()
...
void CRegioneForm::DoDataExchange(CDataExchange* pDX)
{
	static UINT nID_Numero [3] = { IDC_NUM_1, IDC_NUM_2, IDC_NUM_3 };
	static UINT nID_Regione[3] = { IDC_REGIONE_1, IDC_REGIONE_2, IDC_REGIONE_3 };
	static UINT nID_Qta    [3] = { IDC_QTA_1, IDC_QTA_2, IDC_QTA_3 };
	...
	if (m_pEsamiView->m_pEsamiSet->IsOpen())
	{
		for(int i = 0; i < m_nIndexPage; i++)
		{
			...
			DDX_Text(pDX, nID_Qta[i], m_ctrLine[m_nIndexLine+i].m_Set.m_lQuantita);
		}
	}
...
void CRegioneForm::CreateFormEx(CWnd* pParent, CWnd* pParentFrame, CEsamiView* pEsamiView, long lColore, long lIDFaseBlocco)
{
	if (pEsamiView->m_pDynForm[sub_regione] == NULL)
	{
		pEsamiView->m_pDynForm[sub_regione] = new CRegioneForm(NULL, pEsamiView);
		if (pEsamiView->m_pDynForm[sub_regione] != NULL)
		{
			pEsamiView->m_pDynForm[sub_regione]->m_pParentFrame = pParentFrame;
			pEsamiView->m_pDynForm[sub_regione]->m_lColore = lColore;
			pEsamiView->m_pDynForm[sub_regione]->m_lIDFaseBlocco = lIDFaseBlocco;
			
			CRect rectFrame;
			pParentFrame->GetWindowRect(&rectFrame);
			pEsamiView->m_pDynForm[sub_regione]->CreateChildForm(CRegioneForm::IDD, pParent, rectFrame);
		}
	}
}

HBRUSH CRegioneForm::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
...
BOOL CRegioneForm::OnInitDialog() 
{
	...
	AddAnchor(IDC_NUM_1, CSize(0,50), CSize(0,50));
	AddAnchor(IDC_NUM_2, CSize(0,75), CSize(0,75));
	AddAnchor(IDC_NUM_3, CSize(0,100), CSize(0,100));
	AddAnchor(IDC_REGIONE_1, CSize(0,50), CSize(100,50));
	AddAnchor(IDC_REGIONE_2, CSize(0,75), CSize(100,75));
	AddAnchor(IDC_REGIONE_3, CSize(0,100), CSize(100,100));
	AddAnchor(IDC_QTA_1, CSize(100,50), CSize(100,50));
	AddAnchor(IDC_QTA_2, CSize(100,75), CSize(100,75));
	AddAnchor(IDC_QTA_3, CSize(100,100), CSize(100,100));

	GetDlgItem(IDC_SURE_00)->SetFont(&theApp.m_fontBold, TRUE);

	theApp.LocalizeDialog(this, CRegioneForm::IDD, "RegioneForm");
	return TRUE;
}

void CRegioneForm::ModificaForm()
{
	for(int i = 0; i < m_nIndexPage; i++)
		m_ctrlRegione[i].SetParam(&m_ctrLine[m_nIndexLine+i].m_Set.m_lCodice, m_pEsamiView);
}

LRESULT CRegioneForm::OnFormAnnullaEsame(WPARAM wParam, LPARAM lParam)
{
	return OnFormReset(wParam, lParam);
}

LRESULT CRegioneForm::OnFormLoad(WPARAM wParam, LPARAM lParam)
{
	OnFormReset(0, 0);
	GoToFirst();
	...
	if (m_pEsamiView->m_pEsamiSet->IsOpen() && !m_pEsamiView->m_pEsamiSet->IsFieldNull(&m_pEsamiView->m_pEsamiSet->m_lContatore))
	{
		...
		if (setRegione.OpenRecordset("CRegioneForm::OnFormLoad"))
		{
			int nCounter = 0;
			...
			setRegione.CloseRecordset("CRegioneForm::OnFormLoad");
		}
	}
...
LRESULT CRegioneForm::OnFormReadOnly(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CRegioneForm::OnFormReset(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CRegioneForm::OnFormSave(WPARAM wParam, LPARAM lParam)
{
	CRegioneSet setTemp;
	setTemp.SetOpenFilter("Contatore=0");
	if (setTemp.OpenRecordset("CRegioneForm::OnFormSave"))
	{
		SaveForm(FALSE, &m_pEsamiView->m_pEsamiSet->m_lContatore, &setTemp, &setTemp.m_lContatore, &setTemp.m_lEsame);
		setTemp.CloseRecordset("CRegioneForm::OnFormSave");
	}

	return 1;
}

LRESULT CRegioneForm::OnFormSearchNew(WPARAM wParam, LPARAM lParam)
{
...
long CRegioneForm::GetCounterSetOfLine(int nCounter)
{
	return m_ctrLine[nCounter].m_nContatore;
}

BOOL CRegioneForm::IsValidSet(int nCounter)
{
	return (m_ctrLine[nCounter].m_Set.m_lCodice != 0);
}

void CRegioneForm::UpdateDataLine(CBaseSet* pBaseSet, int nCounter, BOOL bToSet)
{
...
BOOL CRegioneForm::IsLineDirty(int nCounter)
{
	return (m_ctrLine[nCounter].m_nStato & RECORD_DIRTY);
}

void CRegioneForm::SetLineDirty(int nCounter, BOOL bSetDirty)
{
	if (bSetDirty)
	{
...
void CRegioneForm::OnChangeRegione1()
{
	UpdateData(TRUE);
...
void CRegioneForm::OnChangeRegione2()
{
	UpdateData(TRUE);
...
void CRegioneForm::OnChangeRegione3()
{
	UpdateData(TRUE);
...
void CRegioneForm::OnChangeQta1()
{
	SetLineDirty(m_nIndexLine+0, TRUE);
}

void CRegioneForm::OnChangeQta2()
{
	SetLineDirty(m_nIndexLine+1, TRUE);
}

void CRegioneForm::OnChangeQta3()
{
	SetLineDirty(m_nIndexLine+2, TRUE);
}

long CRegioneForm::GetContenuto()
{
	return sub_regione;
}

BOOL CRegioneForm::CampoObbligatorioValido()
{
	for(int i = 0; i < MAX_OBJ_X_FORM; i++)
...
void CRegioneForm::OnBnClickedBtnGruppi()
{
...
void CRegioneForm::OnBnClickedBtnReset()
{
...
void CRegioneForm::SaveForm(BOOL bPaziente, long* pParamOfFilter, CBaseSet* pSet, long* pCounterSet, long* pParamSet)
{
	BOOL bError;
	CString strFilter;
	...
	for(int nCounter = 0; nCounter < MAX_OBJ_X_FORM; nCounter++)
	{
		...
		if (IsLineDirty(nCounter))
		{
			if (IsValidSet(nCounter))
			{
				...
				if (nContatore)
				{
					...
					/*if (pSet->RecordsetReOpen(bError, strFilter, "Contatore", "CRegioneForm::SaveForm"))
					...
					if (pSet->RecordsetReOpen(bError, strFilter, "Contatore", "CRegioneForm::SaveForm"))
						pSet->DeleteRecordset("CRegioneForm::SaveForm");

					bSave = pSet->AddNewRecordset("CRegioneForm::SaveForm");
				}
				else
				{
					bSave = pSet->AddNewRecordset("CRegioneForm::SaveForm");
		...
		if (bSave)
		{
			...
			if (pSet->UpdateRecordset("CRegioneForm::SaveForm"))
			{
				SetLineDirty(nCounter, FALSE);
			}
			else
			{
				pSet->NoEditRecordset("CRegioneForm::SaveForm");
				
				CString strError;
				strError.Format(theApp.GetMessageString(IDS_SUBFORM_DONT_SAVE), "CRegioneForm::SaveForm", nCounter);
